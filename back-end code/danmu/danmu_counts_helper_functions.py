...
# 输入弹幕文件路径以及划分时间段的长度
def danmu_counts(danmu_csv_path, block_length, time_list_output_path, chose_rate):
    ...
    time_list.to_csv(time_list_output_path, index=False)


# 输出所选时间序列中的弹幕,对接情感处理
def select_danmu(time_list_path, danmu_input_path, danmu_output_path):
    # 初始化需选择的弹幕时间列表界限和去除列表
    upper_bound = []
    lower_bound = []
    index_to_drop = []
    # 读取弹幕和时间
    origin_danmu = pd.read_csv(danmu_input_path, usecols=['elapse_time', 'text'])
    ...
    # 循环筛选每个弹幕来确定所需的弹幕
    danmu_length = len(origin_danmu)
    for i in range(0, danmu_length):
        flag = 0
        # 循环判断每个时间区间
        for j in range(0, list_length):
            if origin_danmu.iloc[i]['elapse_time'] > lower_bound[j] and origin_danmu.iloc[i]['elapse_time'] < upper_bound[j]:
                flag = 1
        # 如果不是所要弹幕
        if flag == 0:
            index_to_drop.append(i)
    selected_danmu = origin_danmu.drop(index_to_drop)
...
# 弹幕时间前推,以减少发弹幕所需的时间误差
def elapse_time_move(danmu_path, move_time):
    tmp_dm = pd.read_csv(danmu_path)
    ...
    tmp_dm.to_csv(danmu_path, index=False)


# &high_quality=1&autoplay=0,
# <iframe src="//player.bilibili.com/player.html?aid=652122775&bvid=BV1Ye4y1c7pZ&cid=1012241558&page=1" scrolling="no" border="0" frameborder="no" framespacing="0" allowfullscreen="true"> </iframe>
# def cid_get_iframe_code(cid):


if __name__ == "__main__":
    # danmu_counts('D:\双创\sample_1\\result\danmu.csv', 1, 'D:\双创\sample_1\\result\\time_list.csv')
    elapse_time_move('D:\双创\sample_1\\result\danmu.csv', 2)
