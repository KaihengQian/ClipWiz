...
def audio_energy(audio, sample_rate, sample_number, block_number, db, table_name):
    ...
    sql = f"ALTER TABLE {table_name} ADD COLUMN energy DOUBLE;"
    try:
        cursor.execute(sql)
        print(f"成功向表 {table_name} 添加列 energy。")
    except pymysql.Error as e:
        print(f"向表 {table_name} 添加列 energy 出错：{e}")

    data = list(zip(processed_energy))
    sql = f"UPDATE {table_name} SET energy = %s WHERE energy IS NULL LIMIT 1"
    try:
        for value in data:
            cursor.execute(sql, value)
            db.commit()
        print(f"成功插入数据到表 {table_name} 中的 energy 列。")
    except pymysql.Error as e:
        db.rollback()
        print(f"插入数据到表 {table_name} 中的 energy 列出错：{e}")

    cursor.close()


def audio_zero_crossings(audio, sample_rate, sample_number, block_number,  db, table_name):
    ...
    sql = f"ALTER TABLE {table_name} ADD COLUMN zero_crossings DOUBLE;"
    try:
        cursor.execute(sql)
        print(f"成功向表 {table_name} 添加列 zero_crossings。")
    except pymysql.Error as e:
        print(f"向表 {table_name} 添加列 zero_crossings 出错：{e}")

    data = list(zip(processed_zero_crossings))
    sql = f"UPDATE {table_name} SET zero_crossings = %s WHERE zero_crossings IS NULL LIMIT 1"
    try:
        for value in data:
            cursor.execute(sql, value)
            db.commit()
        print(f"成功插入数据到表 {table_name} 中的 zero_crossings 列。")
    except pymysql.Error as e:
        db.rollback()
        print(f"插入数据到表 {table_name} 中的 zero_crossings 列出错：{e}")
