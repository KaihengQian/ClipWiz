...
class DanmuHighlights(object):
    def __init__(self, start, end, video_url):
        ...
        s = Service('D:\\edgedriver_win64\\msedgedriver.exe')
        options = Options()
...
def highlights_play(video_url):
    time_list = pd.read_sql(text("select * from final_list"), engine.connect())
    start_list = np.array(time_list['start'])
    end_list = np.array(time_list['end'])
    video_url_true = "https://www.bilibili.com/video/" + video_url
    test = DanmuHighlights(start_list, end_list, video_url_true)
    test.video_play()
