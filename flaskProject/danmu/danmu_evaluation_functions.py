...
def evaluation_function(our_data_path, stamp_data_path, evaluation_path, predict_data_path):  # our_time是一个二维数组 our_time[1][1]代表{our_starttime[1],our_endtime[1]}
    ...
    recall_rate = float(format(sum(score) / sum(weight) * 100, '.2f'))
    precision_rate = float(format(sum(block_intersection) / fulltime(our_start, our_end) * 100, '.2f'))
    if precision_rate + recall_rate == 0:
        return 0
    # F_beta = 2 * precision_rate * recall_rate / (precision_rate + recall_rate)
    F_beta = 2 * precision_rate * recall_rate / ((precision_rate + recall_rate) * 100)
